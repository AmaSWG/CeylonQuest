...

{
    ...
    // =========================================================
    // STORY 9.2 - EXPERIENCE CANCELLATION / REFUND TESTS
    // =========================================================

    [Fact]
    public async Task CancelBooking_Paid_MoreThan48HoursBefore_ReturnsFullRefund()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddDays(5), PaymentStatus.Paid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var kafka = new Mock<IKafkaProducer>();
        var controller = new BookingsController(context, CreateCatalogService(), kafka.Object);
        SetAuthenticatedUser(controller, visitorId);

        var result = await controller.CancelBooking(booking.Id, new CancelBookingRequest { Reason = "Plans changed" });

        Assert.IsType<OkObjectResult>(result);
        var saved = await context.Bookings.SingleAsync(x => x.Id == booking.Id);
        Assert.Equal(BookingStatus.Cancelled, saved.Status);
        Assert.Equal(PaymentStatus.Refunded, saved.PaymentStatus);
        Assert.Equal(100m, saved.RefundPercentage);
        Assert.Equal(saved.TotalAmount, saved.RefundAmount);
        Assert.NotNull(saved.CancelledAt);
        Assert.NotNull(saved.RefundedAt);
    }

    [Fact]
    public async Task CancelBooking_Paid_Between24And48HoursBefore_ReturnsHalfRefund()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddHours(36), PaymentStatus.Paid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var controller = new BookingsController(context, CreateCatalogService(), new Mock<IKafkaProducer>().Object);
        SetAuthenticatedUser(controller, visitorId);

        var result = await controller.CancelBooking(booking.Id, new CancelBookingRequest { Reason = "Schedule changed" });
        ...
        Assert.Equal(saved.TotalAmount * 0.5m, saved.RefundAmount);
    }

    [Fact]
    public async Task CancelBooking_UnpaidEligibleBooking_HasNoMoneyRefund()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddDays(5), PaymentStatus.Unpaid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var controller = new BookingsController(context, CreateCatalogService(), new Mock<IKafkaProducer>().Object);
        SetAuthenticatedUser(controller, visitorId);

        var result = await controller.CancelBooking(booking.Id, new CancelBookingRequest { Reason = "No longer attending" });
    ...
    public async Task CancelBooking_LessThan24HoursBefore_ReturnsBadRequestAndDoesNotCancel()
    {
        ...
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddHours(12), PaymentStatus.Paid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var kafka = new Mock<IKafkaProducer>();
        var controller = new BookingsController(context, CreateCatalogService(), kafka.Object);
    ...
    [Fact]
    public async Task CancelBooking_PastBooking_ReturnsBadRequest()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddDays(-1), PaymentStatus.Paid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var controller = new BookingsController(context, CreateCatalogService(), new Mock<IKafkaProducer>().Object);
        SetAuthenticatedUser(controller, visitorId);

        var result = await controller.CancelBooking(booking.Id, new CancelBookingRequest());
    ...
    [Fact]
    public async Task CancelBooking_AlreadyCancelled_ReturnsBadRequestAndDoesNotPublishAgain()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
    ...
    [Fact]
    public async Task CancelBooking_AlreadyRefunded_ReturnsBadRequestAndDoesNotRefundAgain()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddDays(5), PaymentStatus.Refunded);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var kafka = new Mock<IKafkaProducer>();
        var controller = new BookingsController(context, CreateCatalogService(), kafka.Object);
    ...
    [Fact]
    public async Task CancelBooking_OtherVisitorsBooking_ReturnsNotFound()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var ownerId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(ownerId, DateTime.Now.AddDays(5), PaymentStatus.Paid);
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var controller = new BookingsController(context, CreateCatalogService(), new Mock<IKafkaProducer>().Object);
    ...
    [Fact]
    public async Task CancelBooking_ValidBooking_PublishesCapacityRestorationEvent()
    {
        var options = CreateDatabaseOptions();
        await using var context = new BookingDbContext(options);
        var visitorId = Guid.NewGuid();
        var booking = CreateStoredExperienceBooking(visitorId, DateTime.Now.AddDays(5), PaymentStatus.Paid);
        booking.ParticipantCount = 3;
        context.Bookings.Add(booking);
        await context.SaveChangesAsync();
        var kafka = new Mock<IKafkaProducer>();
        var controller = new BookingsController(context, CreateCatalogService(), kafka.Object);
        ...
        var result = await controller.CancelBooking(booking.Id, new CancelBookingRequest { Reason = "Cancel" });

        Assert.IsType<OkObjectResult>(result);
        kafka.Verify(x => x.PublishAsync<BookingCanceledEvent>(
            "booking.canceled",
            booking.Id.ToString(),
    ...
    public async Task CancelBooking_BookingNotFound_ReturnsNotFound()
    {
        ...
        var result = await controller.CancelBooking(Guid.NewGuid(), new CancelBookingRequest());
    ...
    private static Booking CreateStoredExperienceBooking(Guid visitorId, DateTime startDateTime, PaymentStatus paymentStatus)
    {
        return new Booking
        {
            ...
            VisitorId = visitorId,
            ListingId = Guid.NewGuid(),
            ListingTitle = "Cancellation Test Experience",
            ListingType = "Experience",
            BookingDate = DateOnly.FromDateTime(startDateTime),
            TimeSlot = startDateTime.ToString("hh:mm tt") + " - " + startDateTime.AddHours(2).ToString("hh:mm tt"),
            ParticipantCount = 2,
            UnitPrice = 5000m,
            TotalAmount = 10000m,
    ...
    private static void SetAuthenticatedUser(BookingsController controller, Guid visitorId)
    {
        var claims = new[] { new Claim(ClaimTypes.NameIdentifier, visitorId.ToString()) };
