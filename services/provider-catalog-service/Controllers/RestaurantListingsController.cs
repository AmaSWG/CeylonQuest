...
public class RestaurantListingsController : ControllerBase
{
    ...
    private static RestaurantListingResponse ToDto(
        RestaurantListing r,
    ...
    /// <summary>
    /// Creates a new restaurant listing owned by the
    /// authenticated approved provider.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> Create(
        [FromBody] CreateRestaurantListingRequest request)
    {
        ...
        var listing = new RestaurantListing
        {
            ...
            // Save provider-configured reservation slots
            TimeSlots = request.TimeSlots?.Trim() ?? string.Empty,

            SetMenuDetails = request.SetMenuDetails.Trim(),
            DietaryOptions = request.DietaryOptions.Trim(),

            GroupSizeCategory =
                request.GroupSizeCategory?.Trim()
                ?? "Table for Two",

            SeatingCapacity =
                request.SeatingCapacity > 0
        ...
        return Created(
            $"/api/catalog/restaurant-listings/{listing.Id}",
            ToDto(listing, provider.BusinessName));
    }

    /// <summary>
    /// Retrieves all restaurant listings owned by the
    /// authenticated approved provider.
    /// </summary>
    [HttpGet]
    ...
    /// <summary>
    /// Retrieves a single restaurant listing by ID,
    /// enforcing provider ownership.
    /// </summary>
    [HttpGet("{id:guid}")]
    ...
    /// <summary>
    /// Updates a restaurant listing owned by the
    /// authenticated approved provider.
    /// </summary>
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(
        Guid id,
        [FromBody] UpdateRestaurantListingRequest request)
    {
        if (!ModelState.IsValid)
            return ValidationProblem(ModelState);
        ...
        // Provider can only update their own restaurant
        if (listing.ProviderId != provider.Id)
            return StatusCode(
                403,
                new
                {
                    message =
                        "You can only update your own restaurant listings."
                });
        ...
        // Update reservation time slots
        listing.TimeSlots =
            request.TimeSlots?.Trim() ?? string.Empty;

        listing.SetMenuDetails =
            request.SetMenuDetails.Trim();

        listing.DietaryOptions =
            request.DietaryOptions.Trim();

        listing.GroupSizeCategory =
            request.GroupSizeCategory?.Trim()
            ?? listing.GroupSizeCategory;
    ...
    /// <summary>
    /// Deletes a restaurant listing owned by the
    /// authenticated approved provider.
    /// </summary>
    [HttpDelete("{id:guid}")]
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        // Provider can only delete their own restaurant
        if (listing.ProviderId != provider.Id)
            return StatusCode(
                403,
                new
                {
                    message =
                        "You can only delete your own restaurant listings."
                });
    ...
    /// <summary>
    /// Returns active restaurant listings for public
    /// discovery with optional filtering.
    /// </summary>
    [AllowAnonymous]
    [HttpGet("public")]
        ...
        [FromQuery] string? location)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cuisine))
        {
            ...
            query = query.Where(r =>
                r.CuisineType.ToLower().Contains(c));
        }
        ...
        if (!string.IsNullOrWhiteSpace(location))
        {
            ...
            query = query.Where(r =>
                r.Location.ToLower().Contains(loc));
        }
    ...
    /// <summary>
    /// Identifies the authenticated provider.
    /// </summary>
        ...
        GetApprovedProviderAsync()
    {
        ...
        var provider =
            await _db.Providers.FirstOrDefaultAsync(p =>
        ...
        if (provider is null)
        {
            return (
                null,
                "Only approved providers can manage restaurant listings. " +
    ...
    /// <summary>
    /// Extracts the authenticated user's identity ID
    /// from NameIdentifier or subject claim.
    /// </summary>
