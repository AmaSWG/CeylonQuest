...
public class AvailabilityController : ControllerBase
{
        ...
        [FromQuery] string date)
    {
        ...
        var result =
            await _availabilityService.GetAvailabilityForDateAsync(
                listingId,
                parsedDate);
        ...
        if (result == null)
        {
            return NotFound(new
            {
                message = "Listing not found or is inactive."
            });
        }
    ...
    /// <summary>
    /// Reserves capacity before a booking is created.
    /// Returns 409 Conflict when the requested capacity
    /// cannot be reserved.
    /// </summary>
    [HttpPost("reserve")]
    public async Task<IActionResult> ReserveCapacity(
        [FromBody] ReserveCapacityRequest request)
    {
        ...
        if (parsedDate < today)
        {
            return BadRequest(new
            {
                message = "Cannot reserve capacity for a past date."
            });
        }

        var reserved =
            await _availabilityService.ReserveCapacityAsync(
                request.ListingId,
                parsedDate,
                request.TimeSlot,
                request.GuestCount);
        ...
        if (!reserved)
        {
            ...
            {
                message =
                    "Insufficient capacity or availability slot not found."
            });
        }
        ...
        [FromBody] SetAvailabilityRequest request)
    {
        ...
        if (parsedDate < today)
        {
            ...
            {
                message =
                    "Cannot create or modify availability for past dates."
            });
        }
        ...
        var (listingFound, ownerId) =
            await GetListingOwnerAsync(listingId);
        ...
        try
        {
            await _availabilityService.SetSlotCapacityAsync(
                listingId,
                parsedDate,
                request.TimeSlot,
                request.Capacity);
            ...
            return Ok(new
            {
                message = "Availability capacity updated successfully."
            });
        }
    ...
    public async Task<IActionResult> SimulateBooking(
        [FromBody] SimulateBookingEventRequest request)
    {
        if (!ModelState.IsValid)
        {
            return ValidationProblem(ModelState);
        ...
        await _availabilityService.DeductCapacityAsync(
            request.ListingId,
            parsedDate,
            request.TimeSlot,
            request.GuestCount);
        ...
        {
            message =
                "Booking event simulated and capacity deducted.",
            availability = updated
        });
    }
    ...
    public async Task<IActionResult> SimulateBookingCanceled(
        [FromBody] SimulateBookingCanceledEventRequest request)
    {
        if (!ModelState.IsValid)
        {
            return ValidationProblem(ModelState);
        ...
        await _availabilityService.RestoreCapacityAsync(
            request.ListingId,
            parsedDate,
            request.TimeSlot,
            request.GuestCount);
        ...
        {
            message =
                "Booking cancellation simulated and capacity restored.",
            availability = updated
        });
    }
    ...
    public async Task<IActionResult> SimulateBookingUpdated(
        [FromBody] SimulateBookingUpdatedEventRequest request)
    {
        if (!ModelState.IsValid)
        {
            return ValidationProblem(ModelState);
        ...
        if (!DateOnly.TryParseExact(
                ...
                CultureInfo.InvariantCulture,
                DateTimeStyles.None,
                out var parsedOldDate))
        {
            return BadRequest(new
            {
                message =
                    "Invalid OldDate format. Expected YYYY-MM-DD."
            });
        }
                ...
                out var parsedNewDate))
        {
            ...
            {
                message =
                    "Invalid NewDate format. Expected YYYY-MM-DD."
            });
        }
        ...
        {
            message =
                "Booking update simulated and capacity adjusted.",
            oldDateAvailability = updatedOld,
            newDateAvailability = updatedNew
        });
    }
    ...
    /// </summary>
    private async Task<(bool found, Guid? ownerId)>
        GetListingOwnerAsync(Guid listingId)
    {
        var act =
            await _db.ActivityListings
                .AsNoTracking()
        ...
        if (act != null)
        {
            return (true, act.ProviderId);
        }

        var rest =
            await _db.RestaurantListings
                .AsNoTracking()
